...
function PerformanceMonitor({ visibleEventCount, filteredEventCount, totalEventCount }) {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      setLiveStats((prev) => {
        const prevCounts = JSON.stringify(prev.counts ?? {});
        const nextCounts = JSON.stringify(nextStats.counts ?? {});
        const prevProfiler = JSON.stringify(prev.profiler ?? {});
        const nextProfiler = JSON.stringify(nextStats.profiler ?? {});

        if (prevCounts === nextCounts && prevProfiler === nextProfiler) {
          return prev;
        }
